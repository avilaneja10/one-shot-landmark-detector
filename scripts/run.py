...
def run(calib_img_paths, test_img_paths, calib_lms, test_lms, level="patch",
    ...
    Args:
        ...
        level:           Embedding granularity — "patch", "pixel", or "bilinear".
        ...
        k:               Number of nearest sources to average for CAOS.
    ...
    if level == "patch":
        emb_gen = PatchEmbeddingGenerator(model=model, patch_size=patch_size, normalize=normalize)
    elif level == "pixel":
        emb_gen = PixelEmbeddingGenerator(model=model, patch_size=patch_size, normalize=normalize)
    elif level == "bilinear":
        emb_gen = BilinearEmbeddingGenerator(model=model, patch_size=patch_size, normalize=normalize)
    else:
    ...
    # ==================================================================
    # Phase 3: Test embeddings + eval scores  (ragged list, no padding)
    # ==================================================================
    ...
    # ==================================================================
    # Phase 4: Test labels + self-removed calibration
    # ==================================================================
    test_labels = np.array(get_landmark_indices(test_lms, test_xy_maps))
    del test_xy_maps

    k_used = min(k, len(calib_img_paths))
    ...
    if "fullcaos" in methods:
        logger.warning("fullcaos skipped — reverse score computation is not yet implemented.")
...
def main():
    ...
    parser.add_argument("--methods", nargs="+", default=None,
                        help='CP methods to run (caos scos fullcaos yk_baseline '
                             'yk_adjust yk_split modsel_cp_ub), or "all"')
